...
class Program
{
    ...
    static void Main()
    {
        ...
        //集合Sを取得
        SkipLine(); // ラベル行をスキップ
        var invalidVertexList = ReadIntsFromConsole().ToList();
        ...
        DfsRecursive(s, walk, visited, t, invalidVertexList, invalidVertexList.Contains(s));
    ...
    static void DfsRecursive(int current, List<int> walk, HashSet<int> visited, int t, IReadOnlyList<int> invalidVertexList, bool hasPassed)
    {
        walk.Add(current);
        visited.Add(current);

        if (!hasPassed && invalidVertexList.Contains(current)) hasPassed = true;
        ...
        foreach (var neighbor in graph[current])
        {
            ...
            {
                DfsRecursive(neighbor, walk, visited, t, invalidVertexList, hasPassed);
            }
        }
